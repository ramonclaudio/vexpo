...
- EAS Build / Update / Submit / Metadata. `runtimeVersion: { policy: "fingerprint" }` (auto-bumps on native code changes), branch/channel model, `appVersionSource: "remote"`. ASC API key managed by EAS (`eas credentials -p ios`), no `eas.json` patches. Known fingerprint drift sources (`expo-modules-jsi/apple/Products/`, `react-native-reanimated/apple/`, `react-native-worklets/apple/`) are pinned out via `.fingerprintignore` so local + EAS-recompute agree. Canonical deploy path: `bunx eas workflow:run .eas/workflows/deploy-production.yml` (runs the build on EAS infra, eliminating client-side fingerprint computation entirely).
