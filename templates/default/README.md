...
- EAS Build / Update / Submit / Metadata. Manual `runtimeVersion: "1.0.0"` (bump by hand on native code changes), branch/channel model, `appVersionSource: "remote"`. ASC API key managed by EAS (`eas credentials -p ios`), no `eas.json` patches. The `fingerprint` policy is the theoretical best practice but currently fails across local-vs-EAS due to upstream issues (`expo-modules-jsi`'s `prepare_command` writes non-deterministic `Products/` stubs, and bun version drift between local and EAS produces different installs of reanimated/worklets). Switch back to `{ policy: "fingerprint" }` once those upstream issues land.
