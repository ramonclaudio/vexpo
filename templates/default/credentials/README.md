...
This folder holds the one-time Apple `.p8` downloads until you upload them to EAS. It's gitignored, and only this README is tracked.
...
Both download from App Store Connect as `AuthKey_*.p8`, so keep one at a time here if you want the CLI to find it on its own.
...
# 2. Register + validate it with the CLI (finds this folder on its own):
...
Keep the ASC key here after the upload. `vexpo submit` and `vexpo asc connect` write its path into `eas.json`'s submit profiles, and that only happens while the `.p8` is inside the repo. You end up with two live ASC keys. The key here is for `eas.json` and CLI submits, and the EAS-managed one is for cloud auto-submits.
