...
- The Maestro suite passes. It never has. Maestro plans a folder in reverse alphabetical order, so `auth.yaml` ran last, after the three flows that need the session it creates, and the dev-menu sheet covers the whole app about 13 seconds into every relaunch and never closes by itself, so no screen id resolves while it is up. `scripts/e2e.mjs` owns the flow order now, turns the dev menu off with the `EXDevMenu*` defaults rather than dismissing it per flow, and enrolls a biometric so the delete step gets past Face ID without anyone watching. That also drops the floating gear that sat over page titles in every screenshot. Three flow bugs came out with it. `launch.yaml` only took a screenshot and passed while photographing the dev menu, the search dismiss reads `Close` on one simulator and `Cancel` on another, and the edge swipe back doesn't register everywhere. The EAS job runs against a release build now, `development:simulator` inherits `developmentClient: true` and came up on the server picker with no Metro to reach. `zz-delete-restore.yaml` stays a local check, the simulator that job gets has no enrolled biometric and the job runs Maestro directly, so nothing can enroll one. 4/4 locally on iPhone 17 and iOS 26.5, 3/3 on EAS on iPhone 16 and iOS 18.3.
- The worklets behind the native text fields stop calling a deprecated scheduler. Reanimated 4 renamed `runOnJS` to `scheduleOnRN` and `runOnUI` to `scheduleOnUI`, and `react-native-worklets` marks both old names `@deprecated` in its own typings. Six field handlers and the `setNativeValue` helper moved over. Same behaviour, and the call shape flattens on the way: `runOnJS(fn)(arg)` becomes `scheduleOnRN(fn, arg)`.
- Reduce Motion keeps the cross-fade into onboarding instead of losing it. The welcome screen branched to `animation: "none"` when the setting is on, but the other branch was already `fade`, which is opacity with no travel and is what the setting asks for. Both branches are one `fade` now. Reduce Motion means gentler, not nothing.
- The banner enter and exit animations get built once. The builder chain was being constructed inside `useBannerMotion`, so every render allocated a fresh one, and the update banner re-renders on every download tick. The three variants sit at module scope now and the hook just picks one.
- Two things that were jumping now move. The onboarding progress bar snapped between steps while the page slid under it, and revoking a session dropped its row so everything below cut upward. Both take SwiftUI's `animation()` modifier at 200ms ease-out, and both fall back to the jump under Reduce Motion, since a filling bar and a closing gap are travel either way. `@expo/ui` measures animations in seconds where the rest of the app is in milliseconds, so a `toSeconds` helper sits next to the `Duration` tokens rather than letting a second scale grow beside them.
- Pushes use the platform transition again. The app stack was forcing `slide_from_right` at 300ms where iOS pushes at 350, and the auth stack was on `fade_from_bottom`, so moving forward from sign-in to forgot-password didn't slide and the back swipe had nothing to reverse. Both stacks pass `default` now and let iOS pick the curve and the duration. Reduce Motion still swaps in the 150ms cross-fade through `useMotionScreenOptions`, and the three modal presentations (welcome, restore-account, the auth gate) keep the animations they were given.
- The offline and update banners slide instead of teleporting. Both render `null` when they have nothing to say, so they were popping in and out at full size. Each one sits in an `Animated.View` now that holds the position, entering from the edge it's pinned to and leaving the same way, 200ms in and 150ms out with ease-out both directions. The motion has to live on an RN view around the content rather than on the content: `@expo/ui` exposes no `transition` modifier, so a SwiftUI subtree inside `Host` can't animate its own insertion. Reduce Motion drops the travel and keeps the cross-fade, read through the app's own hook so the in-app preference counts alongside the system setting.
- Haptics fire on outcomes and on value changes, and on nothing else. All 40 `haptics.light()` calls were on plain taps, and most submits buzzed twice for one action, once when the handler started and again on the success or error at the end. 35 are gone, including every tap-then-navigate. Four moved to `selection()`: the two preferences toggles, the password reveal, and the onboarding Next button, which matches what a swipe between the same steps already fired. The email-change branch signalling "code sent" with a light tap moved to `success()`, the same as the other send paths. Tab presses lost theirs too, since iOS's own tab bar fires nothing and it's the highest-frequency action in the app.
- Profile photos upload again. Since SDK 56 the global `fetch` on native is `expo/fetch`, and `normalizeBodyInitAsync` replaces an explicit `Content-Type` with the blob's own `type`. A blob read off a `file://` URI has an empty type, so every avatar POST left with an empty `Content-Type` and Convex answered `400 BadHeader`. Every photo failed, on every account, since the SDK 56 upgrade. The upload sends `arrayBuffer()` bytes now, which leaves the header alone, and a read that fails says so separately from an upload that fails. The 0.3.2 note about the avatar throwing a ConvexError rather than `Upload failed: 413` is what stood in front of the real status: the screen asked for a different photo when no photo would have worked.
- A phone coming back from lock stops sitting on the profile skeleton. The Better Auth bridge returned `null` for any failed `/convex/token` call, and Convex treats one `null` as signed out, clears auth and does not retry (`refetchToken` in `authentication_manager`). The first call after a resume races the radio waking up, so a single dropped request left the socket unauthenticated and every query on the unauthed early-return until something else moved auth. Transient failures retry three times with backoff now, and only a 4xx returns `null`.
- Caught the template up to the SDK 57 patch matrix again, with expo and expo-router at 57.0.16, expo-updates 57.0.17, `@expo/ui` 57.0.13 and ten more. `expo-doctor` was failing its version-matrix check on all thirteen and passes 21/21 now. The toolchain moved to oxlint 1.80 and oxfmt 0.65 in both trees, and jose 6.2.10 in the template.
- `better-auth` and `@better-auth/expo` hold at 1.6.23 for a second reason now. Beyond the TypeScript 6 break noted in 0.3.1, `@convex-dev/better-auth@0.12.5` declares `better-auth >=1.6.11 <1.7.0`, so the released 1.7.1 is out of range until the adapter moves. `@types/node` stays on 22 against `engines.node >= 22.12` and the 22.12 leg of CI, and the template stays on TypeScript 6.
- `tour.yaml` asserts the privacy rows instead of the disclaimer under them. Dropping the analytics toggle swapped a real control for a `HelperText`, and an inner `Text` reaches Maestro's hierarchy by neither `testID` nor copy, so that assert could not pass from the day it landed. `docs/troubleshooting.md` picks up what the run surfaced: copy you can see in the failure screenshot is not necessarily matchable, a folder run drives the flows at once rather than in name order so the two needing `auth.yaml`'s session go red together, a dependent flow run on its own always starts signed out because the runner resets the keychain, and `zz-delete-restore` no-ops without an enrolled simulator biometric.
- The eleven moderate and four high advisories left in the template lockfile stay. Both roots are build-time only, `image-size` under metro and `uuid` under the `xcode` package that `@expo/config-plugins` uses at prebuild, and neither reaches the app binary. The patched `image-size` is a 2.x major against metro's `^1.0.2`, and npm's own fix for the other downgrades expo to 46.
...
[Unreleased]: https://github.com/ramonclaudio/vexpo/compare/v0.3.2...HEAD
