...
- The eleven moderate and four high advisories left in the template lockfile stay. Both come from build-time packages, `image-size` under Metro and `uuid` under the `xcode` package that `@expo/config-plugins` uses at prebuild, and neither reaches the app binary. The patched `image-size` is a 2.x major against Metro's `^1.0.2`, and npm's own fix for the other downgrades `expo` to 46.
