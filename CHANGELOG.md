...
- `convex-helpers` is pinned exact at `0.1.120`. From 0.1.121 it declares `peer convex@^1.43.0`, and the template pins convex at `~1.42.3`. No lockfile ships with a scaffold, so `^0.1.120` resolved to 0.1.122 on every fresh install. `legacy-peer-deps` is on, so npm installed that mismatch without a word. A scaffold now resolves 0.1.120 against convex 1.42.3, and the pair passes strict peer resolution with the flag off. The two move together whenever convex goes to 1.43.
- Caught the template up to the SDK 57 patches expo 57.0.11 expects: expo and expo-router 57.0.11, expo-updates 57.0.12, `@expo/ui` 57.0.9, and seven more. `expo-doctor` was failing its version-matrix check on all eleven.
