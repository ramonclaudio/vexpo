...
import { BOLD, DIM, GREEN, RESET, YELLOW, line, note, plural, section } from "../lib/output.ts";
...
type PhaseSpec = {
  label: string;
  action: string | ((ctx: PhaseContext) => string);
  details: string[] | ((ctx: PhaseContext) => string[]);
};

const cachedOr =
  (skip: string, run: string) =>
  (ctx: PhaseContext): string =>
    ctx.cached && !ctx.force ? skip : run;

const PHASES: Record<StepName, PhaseSpec> = {
  accounts: {
    label: "setup:accounts",
    action: cachedOr("skip (cached)", "run"),
    details: [
  ...
  rebrand: {
    label: "setup:rebrand",
    action: (ctx) =>
      ctx.willRun ? "run (template defaults detected)" : "skip (already rebranded)",
    details: [
  ...
  "asc-key": {
    label: "setup:asc-key",
    action: cachedOr("skip (cached, validated <24h ago)", "run (interactive y/n)"),
    details: [
  ...
  "apple-services-id": {
    ...
    action: cachedOr("skip (cached)", "run (manual web UI break)"),
    details: [
  ...
  "apple-sign-in": {
    label: "setup:apple",
    action: cachedOr("skip (cached, JWT valid)", "run (interactive y/n)"),
    details: [
  ...
  "apple-credentials": {
    ...
    action: cachedOr("skip (cached)", "run (eas-cli interactive wizard)"),
    details: [
  ...
  "apple-asc-link": {
    ...
    action: cachedOr("skip (already connected)", "run (eas-cli interactive)"),
    details: [
  ...
  "apple-eas-rotation-secrets": {
    ...
    action: cachedOr("skip (all 5 set)", "run (mints CONVEX_DEPLOY_KEY)"),
    details: [
...
function describePhase(
  ...
  const ctx: PhaseContext = {
    cached: status === "cached" || status === "live",
    willRun: options.force === true || probe.needs.get(step) === true || status === "missing",
    force: options.force === true,
    fresh: options.fresh === true,
    convexLive: probe.rows.get("convex")?.status === "live",
  };
  const spec = PHASES[step];
  return {
    step,
    label: spec.label,
    action: typeof spec.action === "string" ? spec.action : spec.action(ctx),
    details: typeof spec.details === "function" ? spec.details(ctx) : spec.details,
  };
}
...
export async function printDryRunPlan(
  ...
  section("Summary");
  line(
    `  ${GREEN}${runCount}${RESET} phase${plural(runCount)} would run, ${DIM}${skipCount}${RESET} skipped`,
  );
  line();
