...
- **`runtimeVersion: { policy: "fingerprint" }`.** A native change forces a fresh build because the hash auto-bumps. OTAs can never load against an incompatible binary. Known drift sources between local and EAS Build are pinned out via `.fingerprintignore` (`expo-modules-jsi/apple/Products/`, `react-native-reanimated/apple/`, `react-native-worklets/apple/`); see `docs/ARCHITECTURE.md` for the why. If you ever hit a `Runtime version calculated on local machine not equal to runtime version calculated during build` failure, either extend `.fingerprintignore` with the new drifting directory, or deploy via `bunx eas workflow:run .eas/workflows/deploy-production.yml` which runs the entire build on EAS infrastructure and eliminates client-side fingerprint computation entirely.
