...
App Attest ships as primitives ready to wire, not a live flow. The goal is cryptographic proof that a request came from an unmodified vexpo binary running on a real iOS device with a Secure Enclave. Rate limiting slows attackers, App Attest proves they came from a real device. Two pieces are in the box:

- **Server verifiers in `convex/appAttest.ts`.** `issueChallenge`, `verifyAttestation`, and `verifyAssertion` are all `internalAction`, so nothing on the client can call them yet. `verifyAttestation` implements Apple's full protocol (per "Validating Apps That Connect to Your Server"): it verifies the cert chain from `x5c[0]` through the intermediate to the pinned Apple App Attest Root CA, recomputes the nonce as `SHA256(authData || SHA256(challenge))` and matches it against the leaf cert's `1.2.840.113635.100.8.2` extension, hashes the leaf's public key and compares against the `credentialId` portion of `authData`, verifies the `rpIdHash` matches `SHA256(<TEAM_ID>.<BUNDLE_ID>)`, verifies the AAGUID matches the deployment environment, and verifies the initial counter is zero. On success the public key + counter land in `appAttestKeys` keyed on `keyId`. `verifyAssertion` checks the ECDSA-P256-SHA256 signature over `SHA256(authenticatorData || SHA256(payload))` against the stored key, and bumps a strictly-increasing counter in one mutation so two concurrent assertions can't both win. `appAttestChallenges` is single-use + TTL'd (5 minutes), swept by the hourly `internal.appAttestStore.cleanupChallenges` cron.
- **Client helper in `src/lib/appAttest.ts`.** `attestThisDevice(client)` and `signRequest(client, keyId, payload)` drive `@expo/app-integrity` (`generateKeyAsync`, `attestKeyAsync`, `generateAssertionAsync`) against an `AppAttestClient` the caller provides. The file ships the interface and the device-side flow, not a concrete client.

Wiring it requires two steps: expose a public action (or `httpAction`) over the `internalAction` verifiers so the device can reach them, and implement the `AppAttestClient` interface to call that action. After that, guard the surfaces that care about anti-abuse by verifying `X-App-Attest-KeyId` + `X-App-Attest-Assertion` + the request body and rejecting when verification throws.
