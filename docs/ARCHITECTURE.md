...
1. **Setup orchestration**: `lite` (dev mode), `full` (TestFlight-ready provisioning). Standalone phases: `accounts`, `rebrand`, `review-account`, `convex`, `better-auth`, `resend`. State machine over `.setup-state.json`. Idempotent, drift-aware, resumable. Calls `eas init` / `eas env:push` / `eas credentials:configure-build` as internal setup steps, doesn't expose any of them as standalone `vexpo` commands. `eas integrations:asc:connect` is printed in the post-setup "Next, eas-cli" list for the user to run themselves; eas-cli's wizard handles create-or-pick-key cleanly and wrapping it added no value.
