...
- **EAS Update**: `runtimeVersion: { policy: "fingerprint" }` so OTA bundles are automatically tied to native compatibility. The fingerprint runtime version policy is fragile out-of-the-box because eas-cli computes the fingerprint BEFORE prebuild + pod install locally, while EAS Build's worker computes AFTER (`eas-cli/src/build/build.ts:181` vs `build-tools/src/utils/expoUpdates.ts:103`); the worker enforces strict equality so any drift between the two trees fails CONFIGURE_EXPO_UPDATES. The template pins out the known drift sources via `.fingerprintignore`: `node_modules/expo-modules-jsi/apple/Products/**` (machine-specific stubs from the podspec's `prepare_command`) and `node_modules/react-native-reanimated/apple/**` + `node_modules/react-native-worklets/apple/**` (bun-version-sensitive extracted content). The fully architecturally-clean alternative is `bunx eas workflow:run .eas/workflows/deploy-production.yml`, which runs the entire build (including fingerprint computation) on EAS infrastructure and eliminates the local-vs-EAS asymmetry entirely. `assetPatternsToBeBundled` limits OTA payload to icon + splash. `enableBsdiffPatchSupport: true` because the runtime cost of bsdiff is negligible relative to the bandwidth savings on incremental updates.
