...
- **EAS Update**: manual `runtimeVersion: "1.0.0"` until upstream fingerprint-determinism issues land. The `{ policy: "fingerprint" }` policy is the right design (OTA bundles automatically tied to native compatibility), but currently fails in practice across local-vs-EAS: `expo-modules-jsi`'s podspec `prepare_command` writes non-deterministic `Products/` stubs that change the package directory's hash, and any bun version drift between local (1.3.14) and EAS Build (1.3.8) produces different extracted content for reanimated/worklets. Switch back to fingerprint policy once those land. With manual version, you bump the string by hand when shipping a native change. `assetPatternsToBeBundled` limits OTA payload to icon + splash. `enableBsdiffPatchSupport: true` because the runtime cost of bsdiff is negligible relative to the bandwidth savings on incremental updates.
