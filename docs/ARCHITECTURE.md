...
- **EAS Update**: `runtimeVersion: { policy: "fingerprint" }` so OTA bundles are automatically tied to native compatibility. On current SDK 56 deps the policy is deterministic across developer machines and EAS Build out of the box: precompiled modules + codegen land in `ios/` (which fingerprint ignores), and the `expo-modules-jsi/apple` artifacts that used to drift are covered by `@expo/fingerprint >= 0.19.3`'s default ignore set, so the template ships no `fingerprint.config.js` and no jsi `.fingerprintignore` entry. Real native version bumps still flip the fingerprint via `package.json` + the `expoAutolinkingConfig:ios` JSON, so the safety contract holds. Verified by unpatched EAS builds clearing `CONFIGURE_EXPO_UPDATES`. `assetPatternsToBeBundled` limits OTA payload to icon + splash.
...
- **External services win on disagreement**: `setup.ts`'s `shouldRun()` gates each step on `isStepFresh(state, step, ttl)`. When a step's cache has gone stale, it runs that step's `liveCheck*` probe against the real service (Convex env, EAS env, the ASC link) and re-records the result. Local cache is not the source of truth.
...
`vexpo` exists for what `eas` doesn't do. Scope test for every command: does it help an empty directory become a first shipped, authenticated, backed iOS app? If it only matters once you have live users, it's out.

1. **Setup orchestration**: `lite` (dev mode), `full` (TestFlight-ready provisioning). Standalone phases: `accounts`, `rebrand`, `review-account`, `convex`, `better-auth`, `resend`. State machine over `.setup-state.json`. Idempotent, drift-aware, resumable. Spawns `eas init` / `eas env:push` / `eas credentials:configure-build` / `eas integrations:asc:connect` as internal setup steps. Of these, only the ASC link is also exposed standalone, as `vexpo asc:connect`. The ASC API key flows through to both `credentials:configure-build` and `integrations:asc:connect` via `EXPO_ASC_API_KEY_*` env vars pre-set from cached state. Notably, `integrations:asc:connect` is invoked with only `--bundle-id` (not `--api-key-id`, which matches against EAS-uploaded key resources rather than Apple-side identifiers).
...
4. **The last ASC mile to a first ship**: `testflight groups`, `testflight testers`, `testflight invite/whats-new` (eas-cli hands a build to TestFlight and stops there), plus the `asc:privacy` and `asc:accessibility` nutrition labels (`show` + `lint`) Apple requires before a submission clears review. Post-launch ASC ops (customer reviews, IAP sandbox testers, version and phased-release management) are deliberately out: they only matter once you have live users, and "eas-cli doesn't expose it" alone never justified adding a command.
...
- **OTA bundle size**: ~600 KB minified + brotli for a typical screen tree.
