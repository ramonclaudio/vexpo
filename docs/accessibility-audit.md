...
| Status banners announced in a timely, non-disruptive way              | pass. `fail()` and `succeed()` announce, and a repeat is heard as a repeat                                                           |
| Element type and state spoken                                         | pass by construction. `@expo/ui` maps onto SwiftUI controls, which carry their own traits                                            |
...
| Apple asks                                         | Verdict                                                                                                                                                                |
| -------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Text reaches 200% or the system maximum            | pass. `useDynamicFont` resolves every size through `Font.custom(relativeTo:)`, so it scales to AX5                                                                     |
...
The five rows marked not verified all need the same thing: a walk through
Apple's common tasks, which for this app are first launch, sign in, settings and
the profile and session screens, with the feature switched on, on a device. Plus
one grayscale pass and one Bold Text pass.
