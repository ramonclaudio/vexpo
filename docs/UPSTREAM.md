...
Not wired yet, on purpose. It sits in the `@expo/ui` Unpublished changelog, which ships as the next SDK 56 patch (`56.0.16`+), expected any day, not as SDK 57. The template pins `@expo/ui` at `~56.0.15`, so that patch arrives with a routine `expo install --fix`, no canary and no `patch-package` fork. The clamp has a real home the moment it does: the OTP fields (24pt monospaced with `kerning(8)` in a fixed-height capsule that can't wrap), the segmented auth toggle, and the "This device" session badge, all fixed-geometry controls that clip rather than reflow at the largest sizes. Everything else already absorbs Dynamic Type through `minHeight` wrapping and the `#46007` `textStyle` scaling.
...
Not wired. It merged into `expo/expo` main and sits in the `@expo/ui` Unpublished changelog (the same next SDK 56 patch as `#46540`), and the template already gets the same VoiceOver outcome through `accessibilityLabel("")` on its decorative leading icons, the pattern documented at `src/components/ui/skeleton.tsx` and applied in the 0.1.1 accessibility audit. Once it lands in a stable `@expo/ui`, `accessibilityHidden(true)` becomes the more honest call for those icons, a semantic swap with no behavior change.
